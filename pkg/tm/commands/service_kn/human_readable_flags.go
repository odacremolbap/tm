...
import (
	...
	kncommands "knative.dev/client/pkg/kn/commands"
...
func printKService(kService *servingv1.Service, options hprinters.PrintOptions) ([]metav1beta1.TableRow, error) {
	...
	age := kncommands.TranslateTimestampSince(kService.CreationTimestamp)
	conditions := kncommands.ConditionsValue(kService.Status.Conditions)
	ready := kncommands.ReadyCondition(kService.Status.Conditions)
	reason := kncommands.NonReadyConditionReason(kService.Status.Conditions)
